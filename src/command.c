...
{
	(!ft_strncmp("cd ", cmd, 3)) ? cd_bin(tab, env) : 1;
/*	
	(!ft_strncmp("env ", cmd, 4)) ? env_bin(tab, env) : 1;
	(!ft_strncmp("echo ", cmd, 5)) ? echo_bin(tab, env) : 1;
	(!ft_strncmp("exit ", cmd, 5)) ? exit_bin(tab, env) : 1;
	(!ft_strncmp("setenv ", cmd, 7)) ? senv_bin(tab, env) : 1;
	(!ft_strncmp("unsetenv ", cmd, 9)) ? uenv_bin(tab, env) : 1;
*/
	return (1);
}
